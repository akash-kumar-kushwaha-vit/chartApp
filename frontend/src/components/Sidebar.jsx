...
const Sidebar = () => {
  ...
  return (
    <>
      {/* Mobile-app style header: taller, safe-area aware */}
      <div className="px-4 py-3 bg-[#f0f2f5] dark:bg-[#202c33] flex justify-between items-center border-b border-[#d1d7db] dark:border-[#202c33] flex-shrink-0 relative z-20"
      ...
      {/* Contacts List — with bottom safe-area padding for home indicator */}
        ...
        {users.length === 0 ? (
          ...
          users.map((user) => (
            <button
              ...
              /* Min height 72px — native app standard for list rows */
              className={`w-full p-0 flex items-center transition-colors cursor-pointer min-h-[72px] active:bg-[#e8eaed] dark:active:bg-[#2a3942] ${
                selectedUser?._id === user._id
                  ? "bg-[#f0f2f5] dark:bg-[#2a3942]"
                  : "hover:bg-[#f5f6f6] dark:hover:bg-[#202c33]"
              }`}
            >
              {/* Avatar — 52px for mobile readability */}
              <div className="pl-4 pr-3 py-2 flex-shrink-0">
                <div className="relative">
                  {user.avtar ? (
                    <img src={user.avtar} alt={user.username} className="w-[52px] h-[52px] rounded-full object-cover" />
                  ) : (
                    <div className="w-[52px] h-[52px] rounded-full bg-[#dfe5e7] dark:bg-[#667781] flex items-center justify-center flex-shrink-0">
                  ...
                  {!user.isGroup && onlineUsers.includes(user._id) && (
                    <span className="absolute bottom-0.5 right-0.5 w-3 h-3 bg-[#00a884] border-[2.5px] border-white dark:border-[#111b21] rounded-full" />
                  )}
                </div>
              </div>

              {/* Text content */}
              <div className="flex-1 text-left min-w-0 pr-4 py-3 border-b border-[#f0f2f5] dark:border-[#202c33] self-stretch flex flex-col justify-center">
                <div className="flex justify-between items-center mb-0.5">
                  <span className="text-[16px] font-medium text-[#111b21] dark:text-[#e9edef] truncate leading-tight flex items-center gap-1.5">
                    {user.fullName}
                    {user.isBlocked && (
                  ...
                  <div className="flex items-center gap-1.5 ml-2 flex-shrink-0">
                    ...
                    {unreadCounts[user._id] > 0 && selectedUser?._id !== user._id && (
                      <div className={`min-w-[22px] h-[22px] px-1.5 rounded-full flex items-center justify-center ${
                        user.isMuted ? "bg-[#8696a0]" : "bg-[#00a884]"
                      }`}>
                        <span className="text-[12px] text-white font-bold leading-none">{unreadCounts[user._id]}</span>
                      </div>
                    )}
                  </div>
                </div>
                <div className="text-[13px] text-[#667781] dark:text-[#8696a0] truncate leading-snug">
                  {user.isGroup ? "Group · " + user.username : `@${user.username}`}
                </div>
              </div>
            </button>
          ))
